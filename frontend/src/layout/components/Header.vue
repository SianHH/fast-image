...
const moreOptions = computed(() => {
  let options = []
  options.push({
    label: localStore().darkTheme ? '浅色模式' : '深色模式', key: 'theme', icon: () => {
      return h(IconTheme, {}, {})
    }
  })
  options.push({
...
const moreSelect = (value) => {
  switch (value) {
    case 'theme':
      localStore().darkTheme = !localStore().darkTheme
      return;
    case 'logout':
...
<template>
  <div class="header-container">
    <div class="header-title">FAST IMAGE</div>
    <n-divider vertical v-if="!menuCollapse"></n-divider>

    <div style="flex: 1"></div>

    <n-divider vertical v-if="!menuCollapse"></n-divider>

    <n-dropdown :options="moreOptions" @select="moreSelect">
      <n-icon :size="20">
